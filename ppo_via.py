...
class Agent:
    ...
    def learn(self):
        ...
        states_arr, actions_arr, old_probs_arr, vals_arr,\
            rewards_arr = self.memory.sample()
        # 计算GAE
        values = vals_arr[:]
        ...
        states = torch.tensor(states_arr, dtype=torch.float).to(self.device)
        # print(old_probs_arr)
        old_probs =old_probs_arr[0]
        # print(old_probs)
        # old_probs = torch.tensor(old_probs_arr).to(self.device)
        actions = torch.tensor(actions_arr).to(self.device)
